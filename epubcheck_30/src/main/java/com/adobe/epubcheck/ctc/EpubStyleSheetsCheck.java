...
public class EpubStyleSheetsCheck implements DocumentValidator{
	...
	public EpubStyleSheetsCheck(Report report, ZipFile zip,  EPUBVersion version) 
	{
		this.zip = zip;
	...
	public boolean validate()
	{
		...
		for(int i=0;i<epack.getManifest().itemsLength();i++)
		{
			...
			if(isValidMediaType(validTypes, itemEntry.getMediaType()))
			{
						...
						LinkTagHandler h = new LinkTagHandler();
						
						parser.parseDoc(fileToParse,h);
						h.checkForMultipleStyleSheets();
													
								if(h.getLinksTagsCount()>1)
								{
									
									CheckingReport.addCheckMessage(itemEntry.getHref(),
											String.valueOf(-1),
											String.valueOf(-1),
											"Multiple CSS for document found ("+h.getStyleSheetsCount()+" stylesheets, "+h.getAlternateStyleSheetsCount()+" alternate stylesheet) ","CTC-049");
