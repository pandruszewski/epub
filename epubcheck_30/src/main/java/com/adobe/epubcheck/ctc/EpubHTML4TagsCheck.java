...
import com.adobe.epubcheck.ctc.xml.HTML4TagHandler;
...
public class EpubHTML4TagsCheck implements DocumentValidator {
	...
	public  EpubHTML4TagsCheck(ZipFile zip, Document  packageMainDocument, Report report, String pathRootFile)
	{
		this.zip=zip;
		this.report=report;
	...
	public  EpubHTML4TagsCheck(EpubPackage epack, Report report)
	{
		...
		for(int i=0;i<epack.getManifest().itemsLength();i++)
		{
			...
			if(isValidMediaType(vtsd,mi.getMediaType()))
			{
				try {
					...
					HTML4TagHandler html4Handler = new HTML4TagHandler(mi.getHref());
					String fileToParse="";
					...
					parser.parseDoc(fileToParse, html4Handler);
